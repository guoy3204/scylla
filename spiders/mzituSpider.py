...
def download_pic(url, title, mm_type, refer):
    ...
    if not os.path.exists(imgDir + mm_type):
        os.mkdir(imgDir + mm_type)
    if not os.path.exists(imgDir + mm_type + "/" + title):
        os.mkdir(imgDir + mm_type + "/" + title)
    with open(imgDir + mm_type + "/" + title + "/" + str(uuid.uuid1()) + ".jpg", 'wb') as fs:
        fs.write(response.content)
